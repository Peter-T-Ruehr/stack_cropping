...
if(type == "Skyscan_1272"){
	File.makeDirectory(source_dir+dir_sep+"recostuff");
	//clean file list
	...
	for(k=0; k<file_list_unsorted.length; k++){
		...
		if(endsWith(curr_file, "log") || endsWith(curr_file, "bmp") || endsWith(curr_file, "spr.tif") || endsWith(curr_file, "pp1.tif") || endsWith(curr_file, "pp2.tif")){  // 
			print(curr_file);
			
			File.rename(source_dir+dir_sep+curr_file, source_dir+dir_sep+"recostuff"+dir_sep+curr_file)
			print(source_dir+dir_sep+"recostuff"+dir_sep+curr_file);
		}
		else {
...
// define log file path and log file name (not possible before because ROI name was unkown)	
...
if(crop_rot_xy==true){
	...
	if(use_ROI_file == true){
		...
		*/
		...
		//read the line points of rotated lines (line_strich_AB&CD) from log file
	...
	//DO THIS IF CROPPING AND ROTATING AND ROI FILE IS NOT LOADED: DEFINE LINES
		...
		//get rotation angle from first line
...
if(save_as_stack == true){
	...
	//else {
		print("Saving  stack images...");
		run("Image Sequence... ", "format=TIFF dir="+target_dir+" name="+specimen_number+"_"+species_name+"_"+ROI_name);
	//}
